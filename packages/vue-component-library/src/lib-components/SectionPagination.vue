...
function handlePageChange(item: number) {
    if (props.initialCurrentPage && props.pages) {
...
function generateLink(pageNumber: number) {
    ...
    if (props.generateLinkCallback) {
        // if there are queryParams in route & props.generateLinkCallback prop provided
        if (queryParams)
            return props.generateLinkCallback(pageNumber, queryParams)

        // else if props.generateLinkCallback prop provided
        return props.generateLinkCallback(pageNumber)
    }
...
function generateLeftPages() {
    if (props.pages && maxPages) {
        let start = 1
        // stop at either maxPages or total props.pages, whichever is lesser
        let stop = Math.min(maxPages.value, props.pages)
        ...
        if (currPage.value === 1) {
            // add 1 more button to the end because 'prev' button is hidden, unless thay would exceed total props.pages
            stop = Math.min(stop + 2, props.pages)
        }

        leftPages.value = []
        for (let i = start; i <= stop; i++) leftPages.value.push(i)
    }
}
function setPaginationMaxPages(width: number) {
    // fail gracefully with 10 as a the default
    if (!props.initialCurrentPage || !props.pages) return 10
    ...
    const itemWidth = Math.ceil(
        buttonWidth + Number.parseInt(buttonMargin) * 2 + 1
    ) // we add 1 to give us a little leeway
    const prevButtonWidth = Math.ceil(
        document.getElementsByClassName("previous")[0].getBoundingClientRect()
            .width + 10
    )
    const nextButtonWidth = Math.ceil(
        document.getElementsByClassName("next")[0].getBoundingClientRect()
            .width + 10
    )
    ...
    const MaxButtons = Math.max(
        ...
        Math.floor(
            +(
                (width - (prevButtonWidth + nextButtonWidth + itemWidth * 2)) /
                itemWidth
            ).toFixed(2)
        )
    )
...
const classes = computed(() => {
    return [
        "section-pagination",
        theme?.value || "",
        props.previousTo === "" ? "first-page" : "",
        props.nextTo === "" ? "last-page" : "",
        {
            "next-prev-only":
                (!props.initialCurrentPage || !props.pages) &&
                props.previousTo &&
                props.nextTo,
        },
    ]
...
const isNotFirstPage = computed(() => {
    return props.initialCurrentPage && props.pages && currPage?.value !== 1
})
const isNotLastPage = computed(() => {
    return (
        props.initialCurrentPage &&
        props.pages &&
        currPage?.value !== props.pages
    )
})
...
watch(
    () => props.pages,
    () => {
        // regenerate props.pages when props.pages change
        generateLeftPages()
    },
    { immediate: true }
)
watch(
    () => props.initialCurrentPage,
    (newVal) => {
        // set current page when props.initialCurrentPage changes
...
onMounted(() => {
    ...
    if (!props.initialCurrentPage || !props.pages) return

    currPage.value = props.initialCurrentPage
    ...
    nextTick(() => {
        ...
        watch(
            ...
            () => {
                ...
                maxPages.value = setPaginationMaxPages(
                    paginationWidth
                ) as number
                generateLeftPages() // then generate buttons representing props.pages
            },
            { immediate: true }
        )
    })
...
<template>
    <div
        ...
        :class="classes"
        role="navigation"
        aria-label="page list navigation"
    >
        ...
        <SmartLink
            v-else-if="isNotFirstPage"
            class="previous"
            :to="generateLink(parsedPrevTo)"
            @click="handlePageChange(parsedPrevTo)"
        >
            <SvgIconArrowRight class="previous-svg" />
            <div class="underline-hover">Previous</div>
        </SmartLink>
        ...
        <div
            v-if="initialCurrentPage && pages"
            class="pagination-numbers-container"
        >
            <div class="pagination-numbers">
                <span v-if="currPage > maxPages" class="page-list-first">
                    <SmartLink
                        :class="`pButton${
                            1 === currPage ? ' ' + 'pButton-selected' : ''
                        }`"
                        :active="currPage === 1"
                        :to="generateLink(1)"
                ...
                <span v-if="currPage > maxPages" class="page-list-truncate"
                    >...</span
                >
                <SmartLink
                    ...
                    :key="item"
                    :class="`pButton${
                        item === currPage ? ' ' + 'pButton-selected' : ''
                    }`"
                ...
                <span
                    v-if="
                        leftPages.length < pages &&
                        leftPages.indexOf(pages) === -1
                    "
                    class="page-list-truncate"
                    >...</span
                >
                <span
                    v-if="
                ...
                >
                    <SmartLink
                        :class="`pButton${
                            pages === currPage ? ' ' + 'pButton-selected' : ''
                        }`"
                        :active="currPage === pages"
                        :to="generateLink(pages)"
        ...
        <SmartLink
            v-else-if="isNotLastPage"
            class="next"
            :to="generateLink(parsedNextTo)"
            @click="handlePageChange(parsedNextTo)"
        >
            <div class="underline-hover">Next</div>
            <SvgIconArrowRight class="next-svg" />
        </SmartLink>
